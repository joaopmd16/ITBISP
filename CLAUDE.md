...
- **Audio recording** uses the standard `MediaRecorder` API (`audio/webm`) — mic-permission prompt is unavoidable (browser security), one-time per session in practice. Both attach-file, record-audio, and screenshot-capture send **immediately** as their own message (no staging/draft area) — simpler UI, at the cost of not being able to add caption text to an attachment in the same message unless typed in the text box first (which *is* included in the same multipart POST if present at send time).
...
- **Frontend UI**: `index.html` has a floating round "Suporte" bubble (`.ticket-bubble`, `position:fixed`, `z-index:900`) that expands into a **non-modal floating widget** (`.ticket-widget`, `z-index:901`, no backdrop — user keeps using the dashboard with the chat open), not a centered modal like `.acc-modal` — that pattern was deliberately not reused here since a chat widget is conventionally a corner panel, not a takeover dialog. `admin.html` reuses the existing `.overlay`/`.modal.wide` pattern instead (4th nav section "Tickets" + a modal for the open thread), consistent with how `admin.html` already handles "Detalhes".
