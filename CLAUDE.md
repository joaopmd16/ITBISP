...
- Both the `/api/` and `/dashboard` proxy blocks have `client_max_body_size 20M;` (nginx defaults to 1MB, which silently 413'd any real ticket-attachment upload before it reached FastAPI — see "Support ticket system" below) and `proxy_http_version 1.1; proxy_set_header Connection "";` (default is HTTP/1.0 to the upstream, which combined with a streamed/chunked FastAPI response could truncate the body partway through — caused real audio-playback cutoffs). If a future feature adds another proxied route, apply the same two settings preemptively rather than rediscovering both issues.
...
### Security review (XSS / SQLi / CSRF / rate limiting / JWT)

Findings from an explicit security pass — fixed unless noted:

- **Stored XSS (fixed, was real and exploitable)**: every user-controlled string rendered via a JS template-literal `innerHTML` assignment must be run through the `_esc()` helper (`index.html`/`admin.html`, near `fmtDataHora`/`fmtData` — HTML-entity-escapes `&<>"'`) before interpolation. This was missed for `nome`/`sobrenome`/`telefone`/`email` in `admin.html`'s user table and detail view, and for ticket `assunto`/message `texto`/attachment filenames on both sides — a malicious `nome` at signup or a ticket `assunto` would execute in **the admin's own browser session** when they viewed the user/ticket list (a direct path to admin-token theft via `localStorage`). Elements set via `.textContent` (e.g. `dNome`, `tkTitulo`, `tkCliente`) were already safe — the DOM auto-escapes text nodes; only `innerHTML`/template-literal-built markup needed the fix. **Any new field added to a rendered list/detail view must go through `_esc()` unless it's coming from a fully closed value set (e.g. a status enum), not free-text user input.**
- **Path traversal + XSS via attachment filename (fixed, was real)**: `_salvar_anexo()` (`main.py`) used the raw uploaded `filename` unsanitized to build both the on-disk path and the DB-stored `anexo_path` — a crafted filename (e.g. containing `../../`) could write outside `backend/uploads/`, and one containing `"`/`<` broke out of the `src="..."` attribute when rendered. Fixed with `_sanitizar_nome_arquivo()` (strips to `os.path.basename` + a `[A-Za-z0-9._-]` allowlist) applied before the file is ever written, plus a resolved-path containment check in `GET /api/tickets/anexos/...` (`caminho_completo.resolve()` must stay under `(UPLOADS_DIR/"tickets").resolve()`) as defense-in-depth against a crafted URL path even if a bad filename ever got stored. Any future upload-accepting endpoint must sanitize the filename the same way before it touches the filesystem — never trust `UploadFile.filename` as a safe path component.
- **SQL injection**: not found — confirmed every query uses `?` placeholders with values passed via the params list, never f-string/`.format()`-interpolated into the SQL text itself. The few `f"...{where}..."` spots (`main.py`, `/api/resumo` and friends) only splice in a WHERE-clause **built entirely from parameterized fragments** (`_multi_like()` takes a hardcoded column name from the caller, never user input, and only puts the actual filter *value* into the params list) — the pattern is safe, but don't add a new filter that splices a raw user-supplied string into the column/operator position.
- **CSRF**: not applicable — auth is Bearer-JWT-in-`Authorization`-header only (`localStorage`, manually attached by JS on every request), no cookie-based session anywhere (confirmed no `set_cookie`/`Set-Cookie` in the codebase). A malicious third-party page cannot make a victim's browser auto-attach the token (unlike cookies) and cannot read `localStorage` cross-origin, so the classic CSRF vector doesn't apply. `CORSMiddleware` uses `allow_origins=["*"]` with `allow_credentials` unset (defaults `False`) — a safe combination for a token-auth JSON API; do **not** add `allow_credentials=True` alongside the wildcard origin if cookie-based auth is ever introduced, that combination is a real vulnerability.
- **Rate limiting (fixed, was missing entirely)**: no endpoint had any throttling before this pass — `/api/auth/login` was open to unlimited password-brute-force, `/api/auth/registrar` to mass account creation, `/api/auth/esqueci-senha`/`reenviar-verificacao` to email-bombing a victim's inbox. Added `_rate_limit(request, chave, max_tentativas, janela_seg)` (`main.py`, near `_sanitizar_nome_arquivo`) — an in-memory sliding-window limiter keyed by `chave:client_ip`, raising `429`. In-memory is safe here specifically because the systemd unit runs a single uvicorn process with no `--workers` flag (see "Serviço systemd" above) — **if the deploy ever moves to multiple workers or a process manager that forks, this in-memory state stops being shared and the limiter becomes per-worker instead of global; would need Redis or a shared store at that point.** Applied to `login` (10/5min), `registrar` (5/hr), `esqueci-senha` (5/hr), `reenviar-verificacao` (5/hr), `redefinir-senha` (10/hr), `POST /api/usuario/trocar-senha` (10/5min). Ticket creation/messaging endpoints were left unlimited since they already require an authenticated session (the `registrar` limit is the actual chokepoint for that path).
- **JWT**: reviewed, no fix needed — `jwt.decode(..., algorithms=["HS256"])` explicitly pins the algorithm (no "alg: none" or algorithm-confusion risk), `JWT_SECRET` is a required env var (`os.environ["JWT_SECRET"]`, fails loudly if unset rather than silently using a weak default), and `exp` is set and verified by PyJWT automatically on decode. **Known accepted trade-off, not a bug**: there's no token revocation/denylist, so a token issued before a password change or before `admin_desativar`/`admin_excluir_usuario` stays valid for the rest of its `JWT_EXP_DIAS=7` window — `exigir_assinatura_ativa` (main.py) re-checks `usuarios.ativo`/`assinaturas.status` from the DB on every protected request, which is what actually enforces "desativado = cut off now" (see "Admin panel" above), not the JWT itself. A password change does *not* revoke other already-issued sessions/tokens — acceptable for this app's threat model, but worth knowing if it's ever raised as a finding again.

### Support ticket system (chat)

A full support-ticket/chat feature, distinct from everything else in "Admin panel" above:

- **Schema**: `tickets` (id, usuario_id, assunto, status `aberto`|`encerrado`, encerrado_por, criado_em, atualizado_em) + `ticket_mensagens` (id, ticket_id, autor `usuario`|`admin`, texto, anexo_path/anexo_tipo/anexo_nome, `lida` — tracks whether **the other party** has read it, criado_em). A user can have multiple tickets with full history; a closed ticket is permanently read-only for both sides (no reopen) — to continue, open a new ticket.
- **No WebSocket** — both sides poll: the open thread view polls every 4s (`GET /api/tickets/{id}/mensagens` or the admin equivalent), the admin ticket *list* polls every 20s while that tab is active, and `index.html` does a quiet 30s badge-only poll (`carregarMeusTickets()`) even when the widget is closed, so the unread badge stays current without the user opening the widget.
- **Attachments** live on VPS disk at `backend/uploads/tickets/{ticket_id}/{random}_{filename}` (`backend/uploads/` is gitignored, mirrors `backend/cache/`), capped at 15MB (`TAMANHO_MAX_ANEXO`, `main.py`).
  - **nginx `client_max_body_size` must be raised** (set to `20M` in both the `/api/` and `/dashboard` blocks of `/etc/nginx/sites-available/itbismart`) — nginx's default is 1MB, so any real photo/PDF attachment was rejected by nginx *before ever reaching FastAPI* (visible only in `/var/log/nginx/error.log` as `"client intended to send too large body"`, never in `journalctl -u itbi` — check nginx's own error log, not just the app log, when an upload fails with no server-side trace).
  - **`GET /api/tickets/anexos/{ticket_id}/{filename}` is NOT served through a public `StaticFiles` mount** — it checks the requester is either the ticket's owner or `ADMIN_EMAIL`, since attachments (screenshots, audio) can contain sensitive info. Because `<img>`/`<audio src=...>` tags never send a custom `Authorization` header (only `fetch`/`XHR` can), this route is added to `ROTAS_PUBLICAS` (skips the blanket header-auth middleware) and instead accepts the JWT via **either** the `Authorization` header **or** a `?token=` query param, decoded manually inside the endpoint — every `<img>`/`<audio>` `src` built client-side must include `?token=${TOKEN}`, or the browser's native media fetch 401s and the attachment silently fails to render (this exact bug shipped once — a real fetch with the header works fine and masks it during testing via `curl -H Authorization`, so always test via an actual `<img>`/`<audio>` tag, not a manual authenticated request).
  - **Serves the file as one fully-buffered `Response(content=path.read_bytes(), media_type=...)`, not `FileResponse`** — `FileResponse` streams/chunks the body, which combined with nginx defaulting to `proxy_http_version 1.0` (no `proxy_http_version 1.1; proxy_set_header Connection "";` in the site config) truncated audio playback partway through in production (an 8.7s recording confirmed complete on disk via Python's `wave` module still cut off after ~3s in the browser). Since attachments are capped at 15MB, reading the whole file into memory is cheap and sidesteps the chunked-transfer/proxy interaction entirely. If this class of bug resurfaces elsewhere (e.g. PDF/Excel export endpoints), check both ends: nginx `proxy_http_version` first, then whether the response is streamed vs buffered.
- **"Capturar tela" is NOT `getDisplayMedia`** — that API pops a screen/tab picker dialog every single time, bad UX for a one-click screenshot button. Both `index.html` and `admin.html` use **`html2canvas-pro`** (CDN `cdn.jsdelivr.net/npm/html2canvas-pro`, same no-bundler pattern as Chart.js/Leaflet) to render the current page's own DOM to a canvas → blob → upload, with zero permission prompt.
  - **Plain `html2canvas` (the original, not the `-pro` fork) cannot parse `color-mix()`** — this app's CSS uses `color-mix(in srgb, ...)` extensively (glows, shadows, borders), and the original library throws `Attempting to parse an unsupported color function "color"` on every capture attempt. `html2canvas-pro` is a maintained fork adding support for `color-mix()`/`oklch()`/`color()` and is a drop-in replacement (same global `html2canvas` function) — if a similar color-parsing error reappears after a future html2canvas-pro version bump, check that release's changelog for regressions in modern-CSS-color support before assuming the bug is elsewhere.
  - `index.html`'s capture passes `ignoreElements: el => el.id==='map' || el.classList.contains('leaflet-container')` — Leaflet's map tiles are cross-origin images without CORS headers, which taint the canvas and make `canvas.toBlob()` throw a SecurityError if the map is included. `admin.html` has no map, so its capture omits this option.
- **Audio recording** uses the standard `MediaRecorder` API — mic-permission prompt is unavoidable (browser security), one-time per session in practice.
  - **Do NOT hardcode `new MediaRecorder(stream)` with no mimeType** — silently fails/produces empty blobs on browsers without the per-browser default (notably Safari, no `audio/webm` support). Feature-detect via `MediaRecorder.isTypeSupported()` against a candidate list (`audio/webm;codecs=opus`, `audio/webm`, `audio/mp4`, `audio/ogg;codecs=opus`, `audio/ogg`) — see `_escolherMimeTypeAudio()`/`_tkEscolherMimeTypeAudio()`.
  - **Recorded webm/opus blobs are converted to WAV client-side before upload** (`_blobParaWav()`/`_tkBlobParaWav()`: `blob.arrayBuffer()` → `AudioContext.decodeAudioData()` → manually-written PCM WAV encoder `_audioBufferToWavBlob()`/`_tkAudioBufferToWavBlob()`) — raw `MediaRecorder` webm output has no duration in its container metadata until played back once (a Chrome muxer limitation with no client-side fix short of a binary EBML patch), which read as "stuck at 0 seconds" / glitchy seeking to users. WAV's header always has a correct, exact duration, so this is a real fix, not a workaround — falls back to uploading the original blob only if `decodeAudioData` itself throws.
  - **Recording indicator is a live reactive waveform** (`.ticket-onda`/`.tk-onda`, ~10 `<span>` bars), not a plain pulsing dot — driven by a `Web Audio API` `AnalyserNode` (`_iniciarOnda()`/`_tkIniciarOnda()`) hooked to the mic stream via `createMediaStreamSource`, sampling `getByteFrequencyData` on a `requestAnimationFrame` loop and setting each bar's height from real amplitude. A static/non-reactive indicator was explicitly reported as reading like "is this actually recording?" — the bars must visibly react to the user's voice, not just blink on a timer.
  - **The send/► arrow button also stops-and-sends an in-progress recording** (`enviarMensagemTicket()`/`tkEnviarMensagem()` check `_ticketGravador.state==='recording'` first) — users expect the same control that sends a typed message to also finalize a voice message, not just the mic icon.
  - **The "Novo ticket" creation form also has attach/record/screenshot buttons** (`ticketNovoAnexarArquivo()`, `ticketNovoToggleGravarAudio()`, `ticketNovoCapturarTela()`, `index.html` only — admin never creates tickets) — since no `ticket_id` exists yet at this point, these **stage** the file/blob into a `_ticketNovoAnexo` variable with a small removable preview chip instead of uploading immediately; `enviarNovoTicket()` bundles the staged file into the same multipart `POST /api/tickets` call that creates the ticket. This differs from the open-thread composer, which uploads every attachment immediately (there, a ticket already exists to attach to).
- **CRITICAL: the 4s/20s poll must never blindly replace the message-list DOM** — the first implementation had `_renderMensagensTicket()`/`tkRenderMensagens()` do `el.innerHTML = mensagens.map(...).join('')` on every poll tick unconditionally. Since `<audio>`/`<img>` elements are destroyed and recreated whenever their parent's `innerHTML` is reassigned, this silently killed and restarted any attachment mid-playback every ~4 seconds — reported by the user as audio that "plays and then stops" (it was being torn down by the next poll, not actually failing). Fixed by fingerprinting the rendered message-id list (`mensagens.map(m=>m.id).join(',')`) and skipping the re-render entirely when it's unchanged from the last render (`_ticketUltimoRenderFingerprint`/`_tkUltimoRenderFingerprint`, reset to `null` whenever a *different* thread is opened so it always renders fresh on first load). Any future polling UI in this app that re-renders live media/interactive elements needs the same guard — diffing before touching the DOM, not unconditional replacement.
- **Paywall bypass for tickets**: `ROTAS_SEM_EXIGENCIA_ASSINATURA_PREFIXO` (`main.py`, a `startswith()`-checked prefix tuple, not exact-match, to support `/api/tickets/*`'s dynamic path segments) includes `/api/tickets` — a user whose subscription lapsed must still be able to open a support ticket (e.g. about the billing issue itself); the middleware still requires a valid JWT, just not an active subscription, for these paths.
- **Endpoints**: `POST/GET /api/tickets`, `GET/POST /api/tickets/{id}/mensagens`, `POST /api/tickets/{id}/encerrar`, `GET /api/tickets/anexos/{ticket_id}/{filename}` (user side); mirrored under `/api/admin/tickets*` (admin side, `exigir_admin`-gated, no `usuario_id` ownership check).
- **E-mail notification**: `emailing.enviar_notificacao_ticket()` fires (best-effort, swallows failures) to `ADMIN_EMAIL` whenever the **user** creates a ticket or sends a message in one — never when the admin replies. No equivalent notification to the user when the admin replies exists — the user relies on reopening the widget (polling) to see admin responses.
- **`python-multipart`** was added to `requirements.txt` — required by FastAPI for any `Form`/`UploadFile` endpoint; **must be `pip install -r backend/requirements.txt` inside the VPS venv before restarting the service** after this feature's deploy (or any future dependency addition), or the app fails to start.
- **Frontend UI**: `index.html` has a floating round "Suporte" bubble (`.ticket-bubble`, `position:fixed`, `z-index:900`) that expands into a **non-modal floating widget** (`.ticket-widget`, `z-index:901`, no backdrop — user keeps using the dashboard with the chat open), not a centered modal like `.acc-modal`. It has a `.ticket-widget.expandido` state (⤢ button) that grows it to `min(720px,94vw) × min(680px,88vh)` for a bigger view — deliberately **not** added to `admin.html`, which already opens threads in a full `.modal.wide` overlay. `admin.html` reuses the existing `.overlay`/`.modal.wide` pattern instead (4th nav section "Tickets" + a modal for the open thread).
- **Protocol numbers**: every ticket list item/row and thread title on both sides is prefixed `Protocolo #{id}` — just the ticket's own `AUTOINCREMENT` id, which is already a single global sequence shared across every user's tickets (SQLite `AUTOINCREMENT` is table-wide, not per-user) — no separate sequence/table was needed to satisfy "numbering must be shared across all users, not reset per-user."
- **Inline `onclick="..."` attributes that embed `JSON.stringify(...)` must use single quotes around the attribute** (`onclick='fn(${JSON.stringify(x)})'`), matching the pattern already used elsewhere in `admin.html` (e.g. `abrirLiberar`) — `JSON.stringify` always emits double-quoted strings, so a double-quoted `onclick="..."` attribute containing it breaks the HTML at the first embedded `"`, silently corrupting the handler (the click does nothing, or calls the function with garbage args). This shipped once in `index.html`'s ticket-list-item `onclick` and made every ticket unopenable — grep for `onclick="[^"]*JSON\.stringify` before adding new inline handlers to catch it again.
...
| GET/POST | `/api/tickets`, `/api/tickets/{id}/mensagens`, `/api/tickets/{id}/encerrar` | Support ticket system — see "Support ticket system (chat)" above |
| GET | `/api/tickets/anexos/{ticket_id}/{filename}` | Auth-gated ticket attachment download (owner or admin only) |
| GET/POST | `/api/admin/tickets*` | Admin mirror of the ticket endpoints — see "Support ticket system (chat)" above |
