...
- **Attachments** live on VPS disk at `backend/uploads/tickets/{ticket_id}/{random}_{filename}` (`backend/uploads/` is gitignored, mirrors `backend/cache/`), capped at 15MB (`TAMANHO_MAX_ANEXO`, `main.py`). They are **not** served through a public `StaticFiles` mount — `GET /api/tickets/anexos/{ticket_id}/{filename}` checks the requester is either the ticket's owner or `ADMIN_EMAIL` before returning `FileResponse`, since attachments (screenshots, audio) can contain sensitive info.
- **"Capturar tela" is NOT `getDisplayMedia`** — that API pops a screen/tab picker dialog every single time, bad UX for a one-click screenshot button. Both `index.html` and `admin.html` use **`html2canvas`** (CDN, same no-bundler pattern as Chart.js/Leaflet) to render the current page's own DOM to a canvas → blob → upload, with zero permission prompt. If a screenshot ever needs to capture something outside the page's own DOM (e.g. another browser tab), `html2canvas` fundamentally can't do that — would need to fall back to `getDisplayMedia` for that specific case.
- **Audio recording** uses the standard `MediaRecorder` API — mic-permission prompt is unavoidable (browser security), one-time per session in practice. Both attach-file, record-audio, and screenshot-capture send **immediately** as their own message (no staging/draft area) — simpler UI, at the cost of not being able to add caption text to an attachment in the same message unless typed in the text box first (which *is* included in the same multipart POST if present at send time).
  - **Do NOT hardcode `new MediaRecorder(stream)` with no mimeType** — the first implementation did this and recording silently failed/produced empty blobs on browsers that don't support the per-browser default (notably Safari, which has no `audio/webm` support at all). Fixed by feature-detecting via `MediaRecorder.isTypeSupported()` against a candidate list (`audio/webm;codecs=opus`, `audio/webm`, `audio/mp4`, `audio/ogg;codecs=opus`, `audio/ogg`) and picking the first supported one — see `_escolherMimeTypeAudio()`/`_tkEscolherMimeTypeAudio()` in `index.html`/`admin.html`.
  - **Always show a recording-in-progress indicator** (pulsing dot + live `mm:ss` timer, `.ticket-gravando-indicator`/`.tk-gravando-indicator`) — a silent recording with only a color-changed mic icon reads as "broken" to users, who can't tell if it's actually capturing anything.
  - **webm blobs from `MediaRecorder` have no duration in their container metadata** until played back once (a well-known Chrome muxer limitation, not fixable client-side without a binary EBML patch) — so the native `<audio>` player can show `0:00` initially even though the recording is fine. Worked around by tracking elapsed time client-side (`Date.now()` at start/stop) and prefixing the message text with the real duration (e.g. `"🎤 Mensagem de voz (0:07)"`) instead of relying on the audio element's own duration display.
- **Paywall bypass for tickets**: `ROTAS_SEM_EXIGENCIA_ASSINATURA_PREFIXO` (`main.py`, renamed from the old exact-match `ROTAS_SEM_EXIGENCIA_ASSINATURA` tuple to a `startswith()`-checked prefix tuple to support `/api/tickets/*`'s dynamic path segments) includes `/api/tickets` — a user whose subscription lapsed must still be able to open a support ticket (e.g. about the billing issue itself); the middleware still requires a valid JWT, just not an active subscription, for these paths.
...
- **E-mail notification**: `emailing.enviar_notificacao_ticket()` fires (best-effort, swallows failures) to `ADMIN_EMAIL` whenever the **user** creates a ticket or sends a message in one — never when the admin replies (they don't need to notify themselves). No equivalent notification to the user when the admin replies is implemented — out of scope so far, the user relies on reopening the widget (polling) to see admin responses.
- **`python-multipart`** was added to `requirements.txt` — required by FastAPI for any `Form`/`UploadFile` endpoint (none existed in this codebase before tickets); **must be `pip install -r requirements.txt` inside the VPS venv before restarting the service** after this feature's first deploy, or the app fails to start.
- **Frontend UI**: `index.html` has a floating round "Suporte" bubble (`.ticket-bubble`, `position:fixed`, `z-index:900`) that expands into a **non-modal floating widget** (`.ticket-widget`, `z-index:901`, no backdrop — user keeps using the dashboard with the chat open), not a centered modal like `.acc-modal` — that pattern was deliberately not reused here since a chat widget is conventionally a corner panel, not a takeover dialog. It has a `.ticket-widget.expandido` state (toggled by a ⤢ button in the header) that grows it to `min(720px,94vw) × min(680px,88vh)` for a bigger view — the admin side deliberately does **not** get this, since `admin.html` already opens threads in a full `.modal.wide` overlay (no cramped-space problem there to solve). `admin.html` reuses the existing `.overlay`/`.modal.wide` pattern instead (4th nav section "Tickets" + a modal for the open thread), consistent with how `admin.html` already handles "Detalhes".
- **Protocol numbers**: every ticket list item/row and thread title on both sides is prefixed `Protocolo #{id}` (just the ticket's own auto-increment `id`, no separate sequence/table) — purely a display convention, no schema change.
