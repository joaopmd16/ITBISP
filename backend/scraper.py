...
def normalizar_df(df: pd.DataFrame, ano: int, mes: int | None) -> pd.DataFrame:
    ...
    # Garante que coluna A (sql_terreno) seja mapeada mesmo com variações de nome
    ...
    # Normaliza o número SQL (IPTU): pode vir como float (ex: 9.80340016e7)
    # Formato esperado: XXX.XXX.XXXX-X
    if "sql_terreno" in df.columns:
        def normalizar_sql(v):
            if not v:
                return None
            s = str(v).strip()
            # Remove notação científica (ex: "1.00800297E8" → "100800297")
            try:
                if 'e' in s.lower() and '.' in s:
                    s = str(int(float(s)))
            except Exception:
                pass
            # Remove ".0" final de floats sem decimais significativas
            if s.endswith('.0'):
                s = s[:-2]
            # Se já tem o formato correto (XXX.XXX.XXXX-X), mantém
            if re.match(r'^\d{3}\.\d{3}\.\d{4}-\d$', s):
                return s
            # Tenta formatar a partir de dígitos puros (11 dígitos → XXX.XXX.XXXX-X)
            digits = re.sub(r'[^\d]', '', s)
            if len(digits) == 11:
                return f"{digits[:3]}.{digits[3:6]}.{digits[6:10]}-{digits[10]}"
            # Mantém como está se não conseguir formatar
    ...
    for sheet_name in wb.sheetnames:
        ...
        try:
            ...
            if header_idx is None:
                # Planilha sem linha de header (ex: 2024) — dados começam na linha 0.
                # Usa nomes posicionais padrão baseados na estrutura conhecida.
                COLS_POSICIONAIS = [
                ...
                n_cols = len(todas_linhas[0]) if todas_linhas else 0
                raw_headers_pos = list(COLS_POSICIONAIS[:n_cols])
                # Preenche colunas extras com nomes genéricos
                for j in range(len(raw_headers_pos), n_cols):
                    raw_headers_pos.append(f"_col{j}")
                raw_headers = tuple(raw_headers_pos)
                data_rows   = todas_linhas  # sem pular linha de header
            else:
